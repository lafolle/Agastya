...
int main()
{
  ...
  int i,j,k;
  bool present,hetro,homo;
  
  while(scanf("%d", &n)!=EOF){

    int h_cnt=0,htr_cnt=0;
    ...
    while(n--){
      ...
      it=MS.find(k);
      if(it!=MS.end())homo=true;
      else hetro=true;

      if(!strcmp(cmd, "insert")) MS.insert(k);
      else if(it!=MS.end())MS.erase(k);


      if(homo&&hetro)printf("both\n");
      else if(homo&&!hetro)printf("homo\n");
      else if(!homo&&hetro)printf("hetro\n");
      else printf("neither\n");
