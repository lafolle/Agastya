...
int n;
vector<unsigned long long int> c(100001), r(100001);
...
int main()
{
  ...
  while(n){
    ...
    for(i=0;i<n;i++)scanf("%llu", &c[i]);
    for(i=0;i<n;i++)scanf("%llu", &r[i]);

    sort(c.begin(), c.begin()+n);
    sort(r.begin(), r.begin()+n, greater<int>());

    for(i=0;i<n;i++) ans+=c[i]*r[i];
...
// AC ( important to use unsigned long long int )
