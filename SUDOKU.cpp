...
struct State{
  int M[16][16],i,j,k,p,q;
  int depth;
  int nv_x,nv_y; // next vacant pos
  SI s,rst,cst;

  State(){}
  State(int mat[16][16],int d, int nvx,int nvy){
    for(i=0;i<16;i++)for(j=0;j<16;j++)M[i][j]=mat[i][j];
    depth=d;
    nv_x=nvx;
    nv_y=nvy;
  }
...
bool possible(int k, int x, int y, int M[16][16])
{
  int bx=(x/4)*4,by=(y/4)*4,i,j;
  for(i=0;i<16;i++)if(M[x][i]==k)return 0;
  for(i=0;i<16;i++)if(M[i][y]==k)return 0;
...
pii getnextvacant(int x,int y)
{
  ...
  for(;i<16;i++,j=0)
    for(;j<16;j++)
      if(!BRD[i][j])return ii(i,j);
  return ii(-1,-1);
}

void solve(State root)
{
  int i,j,k=0,mat[16][16],x,y;
  stack<State> S;
  ...
  while(!S.empty()){
    ...
    ++k;
    for(i=0;i<16;i++)for(j=0;j<16;j++)mat[i][j]=ts.M[i][j];
